...
[data-testid="stAppViewContainer"] {
    background: radial-gradient(circle at top left, #1a1f2f 0%, #0b0f1a 55%, #070a13 100%);
    color: #f5f7ff;
}
...
.stButton>button {
    background: linear-gradient(90deg, #ff4b4b, #ff8a65);
    color: white;
    border-radius: 10px;
    height: 3em;
    width: 100%;
...
[data-testid="stProgressBar"] > div > div > div {
    background: linear-gradient(90deg, #34d399, #ffb347, #ff2e63);
}
</style>
""",
...
with col1:
    ...
    # Safety alert
    if hb < 7:
        st.warning("Severe anemia detected")
...
if st.button("Predict Risk"):
    input_data = np.array(
        [[age, parity, hb, prev_lscs, induction, prolonged, multiple, bmi, bp, prev_pph, placenta]]
    )
    ...
    if prob >= 0.7:
        ...
        gradient = "linear-gradient(135deg, #ff2e63, #b31217)"
        interpretation = "Active management required"
    elif prob >= threshold:
        risk_label = "Moderate Risk"
        gradient = "linear-gradient(135deg, #ffb347, #ff7a18)"
        interpretation = "Close monitoring advised"
    else:
        risk_label = "Low Risk"
        gradient = "linear-gradient(135deg, #34d399, #059669)"
        interpretation = "Routine care sufficient"
